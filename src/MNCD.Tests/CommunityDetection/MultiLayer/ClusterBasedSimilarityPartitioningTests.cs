...
{
    public class ClusterBasedSimilarityPartitioningTests
    {
        [Fact]
        public void ConnectedTriangles()
        {
            ...
            var communities = new ClusterBasedSimilarityPartitioning()
                .GetCommunities(network, n => new Louvain().Apply(n), 2);
        ...
        public void TestRandom()
        {
            ...
            for (var n = 2; n < 15; n++)
            {
                ...
                var communities = new ClusterBasedSimilarityPartitioning()
                    .GetCommunities(network, n => new Louvain().Apply(n), n);

                var count = communities.SelectMany(c => c.Actors).Count();
                Assert.Equal(n, count);
                Assert.Equal(n, communities.Count);
            }
        }
    }
}
